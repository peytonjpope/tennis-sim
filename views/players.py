def render():    
    ...
    with col1:
        ...
        st.write(f"**Country:** {p.country} {country_to_flag(p.country)}")
    ...
    with col2:
        ...
        st.write(f"**Career Titles:** {len([p.trophies for trophy in p.trophies if 'Champion' in trophy])}")
    ...
    with col1:
        ...
        slams, masters, atp500, atp250, atp_finals, achievements = [], [], [], [], [], []

        for trophy in p.trophies:
            if "Year-End" in trophy:
                achievements.append(trophy)
        ...
        # Achievements
        # No 1
        if p.career_high_rank == 1:
            achievements.append("World No. 1")
        if len(slams) >= 4:
            ao, rg, wim, uso = False, False, False, False
            for slam in slams:
                if "Australian Open" in slam: ao = True
                if "French Open" in slam: rg = True
                if "Wimbledon" in slam: wim = True
                if "US Open" in slam: uso = True
                if ao and rg and wim and uso:
                    achievements = ["Career Grand Slam"] + achievements
                    break
        ...
        levels = ["Slams", "Masters", "ATP 500", "ATP 250", "ATP Finals", "Achievements"]

        gs_tab, m1000_tab, atp500_tab, atp250_tab, atpfinals_tab, achievement_tab = st.tabs(levels)
        with gs_tab:
            if slams:
                for trophy in slams:
        ...
        with atpfinals_tab:
            if atp_finals:
                for trophy in atp_finals:
                    if "Champion" in trophy: st.write(f"{trophy}")
                    # elif "Year-End" in trophy: st.write(f"**{trophy}**")
                    # else: st.write(f"{trophy}")
            else:
                st.write("_None_")
